        ...
        ui.nav_panel(
            ...
            ui.layout_sidebar(
                ...
                ui.panel_main(
                    ...
                    ui.hr(),
                    ui.input_action_button("result4_btn", "Get guidelines-based detailed recommendations"),
                    ui.output_text("result4")
                )
            )
        ),
       ui.nav_panel(
           "How to Use the Application",
               ...
               ui.panel_main(
                   ui.markdown("""
                   ### How to Use the Application
                   1. Enter text in the three input fields.
...
#        print("Compliance statement:", compliance_statement)  # Debug print
    ...
    @reactive.Effect
    @reactive.event(input.result4_btn)
    def get_result4():
        combined_result = f"{result1_text.get()} | {result2_text.get()}"
        compliance_response_1 = create_query_engine("C:/Users/Victor/Desktop/Curent/Python_3_11_10_MA/sepsis_guidelines_chroma_db", 
                                                    "sepsis_management_guidelines")
#        compliance_content = compliance_response.get_content() if hasattr(compliance_response, 'get_content') else str(compliance_response)
        diagnosis_query = f"Provide recommendations for sepsis diagnosis confirmation using SOFA/qSOFA criteria based on the following case {joined_text_value.get()}. Recommendations should be maximally based on the queried database. Avoid duplicates in recommendations. The length of your response should not exceed {response_length2_value.get()} tokens."
        diagnosis_recommendations = compliance_response_1.query(diagnosis_query)
        print("Diagnosis recommendations:", diagnosis_recommendations)  # Debug print

        resuscitation_query = f"Provide recommendations for initial resuscitation and treatment, including fluid therapy and source control for the following case: {joined_text_value.get()}. Recommendations should be maximally based on the queried database. Avoid duplicates in recommendations. The length of your response should not exceed {response_length2_value.get()} tokens."
        resuscitation_recommendations = compliance_response_1.query(resuscitation_query)
#        print("Resuscitation recommendations:", resuscitation_recommendations)  # Debug print

        hemodynamic_query = f"Provide recommendations for hemodynamic support and adjunctive therapy, including vasopressors, oxygen, and ventilation support for the following case: {joined_text_value.get()}. Recommendations should be maximally based on the queried database. Avoid duplicates in recommendations. The length of your response should not exceed {response_length2_value.get()} tokens."
        hemodynamic_recommendations = compliance_response_1.query(hemodynamic_query)
#        print("Hemodynamic recommendations:", hemodynamic_recommendations)  # Debug print

        monitoring_query = f"Provide recommendations for monitoring and follow-up, including frequent reassessment and laboratory and imaging studies for the following case: {joined_text_value.get()}. Recommendations should be maximally based on the queried database. Avoid duplicates in recommendations. The length of your response should not exceed {response_length2_value.get()} tokens."
        monitoring_recommendations = compliance_response_1.query(monitoring_query)
#        print("Monitoring recommendations:", monitoring_recommendations)  # Debug print
        ...
        """
#        print("Detailed recommendations received")  # Debug print

        final_recommendations = f"{detailed_guideline_recommendations}"
#    return sepsis_recommendations, antibiotic_recommendations, compliance_statement, compliance_explanation, 
        result4_text.set(final_recommendations)
